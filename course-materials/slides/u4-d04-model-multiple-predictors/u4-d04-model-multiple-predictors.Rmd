...
- _AIC_ trades off model complexity with goodness of fit.
- log-likelihood must improve by 2 units per additional model parameter
- Seek models with lower AIC
...
.pull-right[
```{r}
glance(book_main_fit)$AIC
glance(book_int_fit)$AIC
```
]
...
- Is AIC lower for int model?

```{r}
glance(book_int_fit)$AIC > glance(book_main_fit)$AIC
```
]
]
